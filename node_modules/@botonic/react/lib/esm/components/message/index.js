...
import { BlobContainer, BlobText, BlobTick, BlobTickContainer, MessageContainer, } from './styles';
...
export const Message = props => {
    ...
    let { type = '', blob = true, sentBy, delay = defaultDelay, typing = defaultTyping, children, enabletimestamps = props.enabletimestamps || props.enableTimestamps, json, style, imagestyle = props.imagestyle || props.imageStyle, isUnread = true } = props, otherProps = __rest(props, ["type", "blob", "sentBy", "delay", "typing", "children", "enabletimestamps", "json", "style", "imagestyle", "isUnread"]);
    ...
    const renderBrowser = () => {
        ...
        const getBlobTick = pointerSize => {
            ...
            const color = pointerSize == 5
                ? getBgColor()
                : getThemeProperty(`message.${userOrBotMessage}.style.borderColor`, COLORS.TRANSPARENT);
        ...
        const resolveCustomTypeName = () => isSentByBot && type === INPUT.CUSTOM
            ? ` ${messageJSON.customTypeName}`
            : '';
        const className = `${type}-${userOrBotMessage}${resolveCustomTypeName()}`;
        return (_jsx(ConditionalWrapper, Object.assign({ condition: animationsEnabled, wrapper: children => _jsx(Fade, { children: children }) }, { children: _jsxs(_Fragment, { children: [_jsxs(MessageContainer, Object.assign({ isSentByUser: isSentByUser, style: Object.assign({}, getThemeProperty(WEBCHAT.CUSTOM_PROPERTIES.messageStyle)) }, { children: [_jsx(MessageImage, { imagestyle: imagestyle, sentBy: sentBy }), _jsxs(BlobContainer, Object.assign({ className: className, bgcolor: getBgColor(), color: isSentByUser ? COLORS.SOLID_WHITE : COLORS.SOLID_BLACK, blobwidth: getThemeProperty(WEBCHAT.CUSTOM_PROPERTIES.botMessageBlobWidth), blob: blob, style: Object.assign(Object.assign(Object.assign({}, getMessageStyle()), style), { opacity: ack === 0 ? 0.6 : 1 }) }, otherProps, { children: [markdown ? (_jsx(BlobText, { blob: blob, dangerouslySetInnerHTML: {
                                            __html: renderMarkdown(textChildren),
                                        }, markdownstyle: getMarkdownStyle(getThemeProperty, isSentByUser ? COLORS.SEASHELL_WHITE : brandColor) })) : (_jsx(BlobText, Object.assign({ blob: blob }, { children: textChildren }))), !!buttons.length && (_jsx("div", Object.assign({ className: 'message-buttons-container' }, { children: buttons }))), Boolean(blob) && hasBlobTick() && getBlobTick(6), Boolean(blob) && hasBlobTick() && getBlobTick(5)] }))] })), timestampsEnabled && (_jsx(MessageTimestamp, { sentBy: sentBy, style: timestampStyle, timestamp: messageJSON.timestamp }))] }) })));
    };
