...
class CoreBot {
    ...
    input({ input, session, lastRoutePath, dataProvider, }) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (!session.__locale)
                session.__locale = 'en';
            if (input.type === models_1.INPUT.CHAT_EVENT) {
                return {
                    input,
                    session,
                    lastRoutePath,
            ...
            if (dataProvider) {
                ...
                // @ts-ignore
                const userEvent = yield dataProvider.saveEvent(Object.assign(Object.assign({}, parsedUserEvent), { userId, eventId: (0, ulid_1.ulid)(), createdAt: new Date().toISOString(), from: models_1.MessageEventFrom.USER, ack: models_1.MessageEventAck.RECEIVED }));
            }
            if (this.plugins) {
                yield (0, plugins_1.runPlugins)(this.plugins, 'pre', input, session, lastRoutePath, undefined, undefined, dataProvider);
            }
            if (this.routes instanceof Function) {
                this.router = new routing_1.Router([
                    ...(yield (0, routing_1.getComputedRoutes)(this.routes, {
                        input,
                        session,
                        lastRoutePath,
                    })),
            ...
            if (this.plugins) {
                yield (0, plugins_1.runPlugins)(this.plugins, 'post', input, session, lastRoutePath, response, messageEvents, dataProvider);
            }
            if (dataProvider) {
                // TODO: save bot responses to db and update user with new session and new params
                for (const messageEvent of messageEvents) {
                    // @ts-ignore
                    const botEvent = yield dataProvider.saveEvent(Object.assign(Object.assign({}, messageEvent), { userId, eventId: (0, ulid_1.ulid)(), createdAt: new Date().toISOString(), from: models_1.MessageEventFrom.BOT, ack: models_1.MessageEventAck.SENT }));
                }
            }
