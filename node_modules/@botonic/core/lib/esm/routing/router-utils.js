...
export function getComputedRoutes(routes, request) {
    return __awaiter(this, void 0, void 0, function* () {
        if (routes instanceof Function) {
            return yield getComputedRoutes(yield routes(request), request);
        }
        for (const [key, route] of Object.entries(routes)) {
            if (route.childRoutes && route.childRoutes instanceof Function) {
                routes[key].childRoutes = yield getComputedRoutes(yield route.childRoutes(request), request);
            }
            else {
                routes[key] = route;
            }
